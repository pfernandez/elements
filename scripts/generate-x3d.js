...
const writeX3DHelpersRuntime = byName => {
  ...
  lines.push('')
  lines.push('import { createTagHelper, withX3DOM } from \'./core/elements-3d.js\'')
  lines.push('')
  for (const e of exportsList) {
    lines.push(`/** @see https://doc.x3dom.org/author/${e.component}/${e.pascal}.html`)
    if (e.exportName !== e.tag) lines.push(` * Exported as \`${e.exportName}\` to avoid colliding with the HTML/SVG \`${e.tag}\` helper.`)
    lines.push(' */')
